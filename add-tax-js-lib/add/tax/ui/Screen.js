sap.ui.define([
     ...
     * 
     * Exemplos:
     * let screx = add.tax.ui.Screen.extend("add.custom.Screen", { 
     *     
     *     <novoMetodo>,
     * 
     *     <metodoReescrito>
     ...
     * });
     * 
     * let scr = new screx().factory(this, 'add-core-taxrm-app', parameters);
     * sap.m.Page({content: scr}).placeAt();
     *******************************************************************************************/

    return sap.m.NavContainer.extend("add.tax.ui.Screen", {
        ...
        _getFields() {
            ...
            {
                field: "BRANCH", prop: { obligatory: false }
            },
            {
        ...
        getData: async function getData() {
            ...
            resp = JSON.parse(await callService.postSync('add/tax', req));
            ...
            if (!this.STRUC && resp && !resp.stop) {
                ...
                if (fields.length > 0) {
                    ...
                    /****************************************************************
                     * Serviço que irá obter propriedades de todas as colunas do grid
                     * 
                     ****************************************************************/
        ...
        scheduler: async function (oEvent) {
            ...
            try {
                ...
                MessageToast.show(AddModels.getI18n(await callService.postSync('add/tax', req)));
