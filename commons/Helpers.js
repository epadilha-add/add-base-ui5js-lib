                ...
                return new Promise((resolve, reject) => {
                    ...
                    this._oDialog = new sap.m.P13nDialog({
                        ...
                        ok: function (e) {
                            var struc = e
                                .getSource()
                                .getModel(that.IDAPP + "tablestruct")
                                .getProperty("/");
                            ...
                            var strucSMapped = strucSorted.map((item, i) => {
                                ...
                                if (!item.COL_POS) item.COL_POS = i + 1;
            ...
            openDialog(that, filterstruct) {
                return new Promise((resolve, reject) => {
                    filterstruct = filterstruct.map(el => {
                        el.selected = el.VISIBLE === "X" ? true : false;
                        el.sectionID = el.SECTION.substring(0, 1);
                        el.sectionName = that
                            .getView()
                            .getModel("i18n")
                            .getResourceBundle()
                            .getText("SECTION_" + el.sectionID);
                        return el;
                    });
                    ...
                    var oTable = new sap.m.Table({
                        ...
                        fixedLayout: false,
                        columns: [new sap.m.Column({ header: new sap.m.Label({ text: "Nome" }) }), new sap.m.Column({ header: new sap.m.Label({ text: "Descrição" }) })],
                        items: {
